...
draft: false
quiz:
  question: "開発・コンパイル用の「重い一時ステージ」から、本番用の「軽量ステージ」へ成果物のみをコピーすることで、最終イメージの容量を大幅に削減しセキュリティを向上させるビルド技術は何でしょうか？"
  options:
    - "キャッシュビルド"
    ...
    - "レイヤーフロー"
  correctIndex: 1
  explanation: "マルチステージビルドは、1つの Dockerfile 内に複数の `FROM` ステージを定義し、あるステージでビルドしたファイル（成果物）だけを次の最終ステージへ引き継ぐことで、ビルドに必要なコンパイラなどの不要な依存関係を排除した極小イメージを作成できる技術です。"
---
