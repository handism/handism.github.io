  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <div className="flex justify-between items-center mb-2">
          ...
          {uaInput !== detectedUa && (
            <button
              ...
              className="flex items-center gap-1 text-xs text-accent hover:underline font-bold cursor-pointer"
            >
        ...
        <div className="flex gap-2">
          <textarea
            ...
            className="flex-1 h-28 p-4 border-2 border-border bg-card text-text rounded-2xl focus:ring-2 focus:ring-accent focus:outline-none font-mono text-xs shadow-[2px_2px_0px_0px_var(--border)] resize-y"
          />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
        {/* Browser */}
        <div className="bg-card p-5 rounded-2xl border-2 border-border shadow-[4px_4px_0px_0px_var(--border)] flex flex-col justify-between">
          <div>
            <span className="text-[10px] font-bold text-accent uppercase tracking-wider block mb-1">
              ブラウザ名 / バージョン
            </span>
            <span className="font-bold text-text text-lg">{parsedResult.browser.name}</span>
          </div>
          <span className="font-mono text-xs text-text/50 mt-2 block break-all">
        ...
        <div className="bg-card p-5 rounded-2xl border-2 border-border shadow-[4px_4px_0px_0px_var(--border)] flex flex-col justify-between">
          <div>
            ...
            <span className="font-bold text-text text-lg">{parsedResult.os.name}</span>
          </div>
          <span className="font-mono text-xs text-text/50 mt-2 block break-all">
            {parsedResult.os.version ? `Version ${parsedResult.os.version}` : 'バージョン情報なし'}
          </span>
        </div>
        ...
        <div className="bg-card p-5 rounded-2xl border-2 border-border shadow-[4px_4px_0px_0px_var(--border)] flex flex-col justify-between">
          <div>
            ...
            <span className="font-bold text-text text-lg flex items-center gap-1.5">
              <Monitor className="w-4 h-4 text-text/40" />
          ...
          <span className="font-mono text-xs text-text/50 mt-2 block">
        ...
        <div className="bg-card p-5 rounded-2xl border-2 border-border shadow-[4px_4px_0px_0px_var(--border)] flex flex-col justify-between">
          <div>
            ...
            <span className="font-bold text-text text-lg">{parsedResult.engine}</span>
          </div>
          <span className="font-mono text-xs text-text/50 mt-2 block">Engine Type</span>
        </div>
      </div>

      {/* コピーボタン */}
      <div className="flex justify-between items-center p-4 bg-secondary border-2 border-border rounded-2xl shadow-[2px_2px_0px_0px_var(--border)]">
