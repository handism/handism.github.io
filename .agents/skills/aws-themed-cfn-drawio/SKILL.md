...
- `draw.io/<name>.drawio`: テンプレートと整合したAWS構成図（mxGraph XML）
- `img/<name>.drawio.svg`: draw.io CLIでエクスポートしたSVG（**必須**）

## 前提ツール

draw.io CLI（v30+）がインストール済みであること。
...
1. 既存の `draw.io/` に類似パターンがあれば、それを読み込んでXMLを改変する
...
1. 生成・更新したファイルパス（drawio / svg / yaml の3点セット）
2. テーマへの反映要点（3-7項目）
...
- AWS 2026 shapeID一覧: [aws-shape-ids.md](references/aws-shape-ids.md)
