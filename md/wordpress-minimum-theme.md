...
```mermaid
graph LR
    A[1. HTMLでマークアップ] --> B[2. テンプレートタグへ変換]
    B --> C[3. CSSで装飾]
    C --> D[4. テーマのインストール]
...
```mermaid
graph TD
    subgraph Static ["静的HTML (index.html)"]
        S1["&lt;title&gt;ブログタイトル&lt;/title&gt;"]
        S2["&lt;a href='ブログのURL'&gt;"]
        S3["&lt;div class='article'&gt;&lt;/div&gt;"]
    end
    subgraph Dynamic ["WordPressテンプレートタグ (index.php)"]
        D1["bloginfo('name') / wp_title()"]
        D2["echo home_url()"]
        D3["the_content()"]
    end
    S1 -. 変換 .-> D1
