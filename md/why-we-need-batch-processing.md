...
![オンライン処理とバッチ処理の選択理由を比較するシステムアーキテクチャ図解](/images/why-we-need-batch-processing-infographic.webp)
...
![オンライン処理とバッチ処理のリソース使用パターンの比較](/images/batch-vs-online-resource.webp)
...
```mermaid
graph TD
    subgraph Online ["オンライン (補償トランザクション)"]
        A1[処理A 成功] --> A2[処理B 失敗]
        A2 --> A3[補償処理A 実行: 状態巻き戻し]
        style A3 fill:#ffebee,stroke:#c62828
    end
    subgraph Batch ["バッチ (一括再実行)"]
        B1[データ読み込み] --> B2[一括処理中にエラー]
        B2 --> B3[全体をロールバックして再実行]
