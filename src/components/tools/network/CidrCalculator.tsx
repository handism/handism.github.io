...
export default function CidrCalculator() {
  ...
  // 32文字のバイナリをオクテット境界に分割して、ネット部・ホスト部のオブジェクト配列を返す
      ...
      <div className="lg:col-span-5 space-y-6">
        <div className="theme-card p-5 md:p-6 space-y-5 border-2 border-border bg-card shadow-[4px_4px_0px_0px_var(--border)]">
          ...
          <div className="space-y-2">
            <label
              htmlFor="ip-input"
              className="block text-xs font-black text-text/60 uppercase tracking-wider"
            >
              IP アドレス (IPv4)
            </label>
            <input
              ...
              className="w-full px-4 py-3 border-2 border-border rounded-xl font-mono text-base font-bold bg-card text-text focus:outline-none focus:ring-1 focus:ring-accent shadow-inner"
            />
          ...
          <div className="space-y-2">
            <div className="flex justify-between items-center">
              ...
              <span className="text-sm font-extrabold text-accent bg-secondary border border-border px-2.5 py-0.5 rounded-lg shadow-sm">
                /{prefix}
              </span>
            </div>
            <input
              ...
              {[
                ...
                <div key={item.key} className="flex justify-between items-center py-3">
                  <div className="min-w-0 pr-4">
                    <span className="text-xs font-bold text-text/50 block">{item.label}</span>
                    <span className="text-sm sm:text-base font-extrabold text-text font-mono truncate block mt-0.5 select-all">
                      {item.val}
                    </span>
                  </div>
                  <CopyButton
