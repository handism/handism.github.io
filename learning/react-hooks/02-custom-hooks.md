...
draft: false
quiz:
  question: "React が自作したフックを正しいフックとして認識できるようにするために守るべき、命名規則のルールはどれでしょうか？"
  options:
    - "フック関数の末尾を Hook で終わらせる"
    - "フック関数の先頭を小文字の use から始め、続く文字を大文字にする"
    - "フック関数の先頭を get で始める"
    - "フック関数名をすべて大文字（スネークケース）にする"
  correctIndex: 1
  explanation: "React のルールおよび ESLint の自動検証を機能させるため、カスタムフックの名前は必ず `use` から始める必要があります（例: `useFetch`, `useLocalStorage`）。"
---
