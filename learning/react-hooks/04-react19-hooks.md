...
draft: false
quiz:
  question: "React 19 で導入された useFormStatus フックを呼び出す際、満たす必要がある重要な配置上のルールはどれでしょうか？"
  options:
    - "サーバーコンポーネント内で直接呼び出さなければならない"
    - "対象となる <form> 要素の内部（内側）にレンダリングされる子コンポーネント内で呼び出す必要がある"
    - "フックの引数に必ず依存配列 [] を渡さなければならない"
    - "非同期関数（async）の中で呼び出さなければならない"
  correctIndex: 1
  explanation: "`useFormStatus` は React Context の仕組みを利用して親のフォームの送信状態を読み取ります。そのため、必ず `<form>` 要素の中にネストされた子コンポーネント内で呼び出す必要があります。同一ファイルで `<form>` を定義した場所と同じコンポーネント内では機能しません。"
---
