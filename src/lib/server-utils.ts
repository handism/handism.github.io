 ...
 */
export function processMetadataList<
  TSource extends { slug: string; raw: string },
  TMeta extends { date?: Date; draft?: boolean },
>(sources: TSource[], parseAndCreateMeta: (slug: string, raw: string) => TMeta): TMeta[] {
  const items = sources.map(({ slug, raw }) => parseAndCreateMeta(slug, raw));
