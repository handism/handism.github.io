...
draft: false
quiz:
  question: "Docker における「イメージ」と「コンテナ」の関係について、最も適切な説明はどれでしょうか？"
  options:
    - "イメージは実行中のコンテナからしか構築できない書き込み可能レイヤーである"
    - "コンテナは「読み取り専用のテンプレートであるイメージ」の上に、書き込み可能レイヤーを載せて動かした実体（実行インスタンス）である"
    - "イメージとコンテナは完全に同じ概念であり、呼び方が環境によって異なるだけである"
    - "イメージを起動するためには、コンテナごとに個別のゲストOSを毎回インストールする必要がある"
  correctIndex: 1
  explanation: "Dockerイメージは、アプリ実行に必要なファイル群が入った「読み取り専用の設計図」です。このイメージを起動すると、その上に薄い書き込み可能なレイヤーが追加され、独立して実行される「コンテナ」となります。"
---
