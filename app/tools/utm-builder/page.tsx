...
export default function UtmBuilderTool() {
  ...
  return (
    <ToolPageLayout
      ...
      description="Google Analytics等でのアクセス解析に用いるカスタムキャンペーンパラメータ（UTM）を追加したURLをミスなく作成します。パラメータプリセット、URLバリデーション、QRコード生成、短縮URLシミュレーションに対応。"
      icon={LinkIcon}
    >
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="theme-card p-6 bg-card border-2 border-border shadow-[4px_4px_0px_0px_var(--border)] dark:shadow-[4px_4px_0px_0px_var(--accent)] space-y-6">
            ...
            <div className="space-y-1.5">
              ...
              <input
                ...
                className={`w-full bg-secondary border-2 rounded-xl px-4 py-3 text-xs text-text focus:outline-none focus:ring-1 focus:ring-accent font-medium ${
        ...
        <div className="lg:col-span-5 space-y-6">
          {/* 生成結果カード */}
          <div className="theme-card p-6 bg-card border-2 border-border shadow-[4px_4px_0px_0px_var(--border)] dark:shadow-[4px_4px_0px_0px_var(--accent)] space-y-5">
                ...
                <div className="space-y-2">
                  <div className="flex justify-between items-center text-[10px] font-bold text-text/60">
                    ...
                    <button
                      ...
                      className="px-2 py-1 rounded bg-secondary border border-border text-[9px] font-bold text-text hover:bg-secondary/80 flex items-center gap-1 cursor-pointer transition-colors animate-none"
                    >
                  ...
                  <textarea
                    readOnly
                    value={generatedUrl}
                    className="w-full font-mono text-[10px] text-text bg-secondary/40 p-3 rounded-xl border border-border overflow-x-auto resize-none h-20 focus:outline-none select-all leading-normal"
                  />
                </div>
                ...
                <div className="space-y-2 border-t border-border pt-4">
                  <div className="flex justify-between items-center text-[10px] font-bold text-text/60">
                    ...
                    {shortUrl && (
                      <button
                        ...
                        className="px-2 py-1 rounded bg-secondary border border-border text-[9px] font-bold text-text hover:bg-secondary/80 flex items-center gap-1 cursor-pointer transition-colors"
                      >
                  ...
                  {shortUrl ? (
                    <input
                      ...
                      readOnly
                      value={shortUrl}
                      className="w-full font-mono text-xs text-accent bg-secondary/40 px-3 py-2.5 rounded-xl border border-border focus:outline-none select-all"
                    />
                  ) : (
                    <button
                      onClick={handleShorten}
                      disabled={isShortening}
                      className="w-full py-2.5 bg-accent hover:opacity-90 disabled:opacity-50 text-white rounded-xl text-xs font-bold flex items-center justify-center gap-1.5 cursor-pointer transition-all"
                    >
                      {isShortening ? (
                        <>
          ...
          {/* QRコードカード */}
          {generatedUrl && (
            <div className="theme-card p-5 bg-card border-2 border-border shadow-[4px_4px_0px_0px_var(--border)] dark:shadow-[4px_4px_0px_0px_var(--accent)] flex flex-col items-center justify-center gap-4">
              ...
              {/* QRコード描画エリア */}
              <div className="bg-white p-2 rounded-2xl border border-border shadow-sm flex items-center justify-center">
                <canvas ref={qrCanvasRef} className="max-w-full" />
              </div>

              <button
                onClick={downloadQrCode}
                className="w-full py-2 border border-border bg-card hover:bg-secondary text-text font-bold text-xs rounded-xl flex items-center justify-center gap-1 cursor-pointer transition-colors"
              >
