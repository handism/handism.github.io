...
コンピュータは $0$ と $1$ のデジタルな世界であり、連続的ではない「離散的」な値を扱います。そのため、離散数学の概念が基礎となります。

### 論理演算 (Boolean Logic & Bitwise Operations)

もっとも身近なのが「論理演算」です。条件分岐だけでなく、ビット単位での高速な操作にも使われます。

- **AND (`&&`, `&`)**: 両方のビット/条件が $1$ のときのみ $1$。
- **OR (`||`, `|`)**: 少なくとも一方のビット/条件が $1$ のときに $1$。
- **NOT (`!`, `~`)**: 真偽やビットを反転させる。
- **XOR (`^`)**: どちらか一方が $1$ のときに $1$。同じ値のときは $0$ になるため、フラグの反転や簡単な暗号化に利用されます。

```typescript
// XOR を使った簡易的なビット反転の例
const flag = 0b1010;
const inverted = flag ^ 0b1111; // 0b0101 になる
```

### 集合論 (Set Theory)

データの集まり（コレクション）を扱う際、数学的な「集合」の考え方が不可欠です。

- **和集合 (Union)**: すべての要素の集まり。
- **積集合 (Intersection)**: 両方に共通する要素の集まり。
- **差集合 (Difference)**: 一方の集合からもう一方に含まれる要素を取り除いたもの。

SQL の `JOIN` や、TypeScript/JavaScript の `Set` オブジェクトによる重複排除などは、まさにこれら集合演算の実装です。
